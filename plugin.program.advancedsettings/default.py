...
def MENU(name):
    addDir("Einstellungen bearbeiten", 'url',499, iconart,'blank','Build your advanced settings file')
    addDirPlayable("Schreibe XML File", 'url',500, iconart,'Feeling brave? Write your advanced settings directly to your userdata directory. You may need to reboot for settings to take effect',ACTIVESETTINGSFILE,'')
    if os.path.exists(ACTIVESETTINGSFILE):
        addDirPlayable("Zeige aktive advancedsettings.xml", 'url',495, '','View your active advanced settings file','','')
        addDirPlayable("Loesche advancedsettings.xml", 'url',490, '','Delete all advanced settings (settings are saved in this addon to be written later). Reboot may be required to take effect','','')
    addDirPlayable("Schreibe die XML-Datei in einen temporaeren Speicherort", 'url',500, '','Play it safe. Write your advanced settings to userdata/plugin.program.advancedsettings/XML_FILES/ directory',editfile,'')
    addDirPlayable("Alle Einstellungen zuruecksetzen", 'url',489, '','Resets all settings saved in this addon only. You will still need to run "Remove advancedsettings.xml" and reboot to remove completely','','')
    addDir("Aktivierte Einstellungen auflisten", 'url',498, '','blank','Check which settings are enabled before writing your xml file')
    setView('movies', 'movies-view')
    xbmc.executebuiltin("Container.SetViewMode(%s)" % VIEW)
	
def buildmenu(name):
    addDir("Fehlerbehebung bei Einstellungen ", 'url',10, '','blank','Define your video settings')
    addDir("Audio/Video Wiedergabe Einstellungen", 'url',11, '','blank','Define your audio settings')
    addDir("Video Bibliothekseinstellungen", 'url',12, '','blank','Define your video library settings')
    addDir("Bibliotheksgrafik", 'url',13, '','blank','Define your library artwork settings')
    addDir("Video and Music Bibliothekseinstellungen", 'url',14, '','blank','Define your video and music library settings')
    addDir("Music Einstellungen", 'url',15, '','blank','Define your music settings')
    addDir("Photos Einstellungen", 'url',16, '','blank','Define your photo settings')
    addDir("Netzwerk Einstellungen", 'url',17, '','blank','Define your network settings')
    addDir("Dateisystemeinstellungen", 'url',18, '','blank','Define your file system settings')
    addDir("Fernbedienung Einstellungen", 'url',19, '','blank','Define your remote control settings')
    addDir("Andere Schnittstelleneinstellungen", 'url',20, '','blank','Define other interface settings')#Unsorted 
    addDir("Unsortiert", 'url',21, '','blank','Unsorted network settings')
    setView('movies', 'movies-view')
    xbmc.executebuiltin("Container.SetViewMode(%s)" % VIEW)
