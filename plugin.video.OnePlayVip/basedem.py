...
#### DESCRIÇÃO VIP ###########################################################
url_b64_vip_descricao = 'aHR0cHM6Ly9wYXN0ZWJpbi5jb20vcmF3LzBaWEhKMDZ1'
url_vip_descricao = base64.b64decode(url_b64_vip_descricao)
#### DIALOGO VIP - SERVIDOR DESATIVADO - CLICK ###################################
url_b64_vip_dialogo = 'aHR0cHM6Ly9wYXN0ZWJpbi5jb20vcmF3LzZ3VGU2S2dk'
...
def addon_log(string):
    if debug == 'true':
       # xbmc.log("[addon.live.SimpleKore Lists-%s]: %s" %(addon_version, string))
        xbmc.log("[OnePlay-%s]: %s" %(addon_version, string))
...
def contador():
    ...
    "Referer": "OnePlayVIP-1.0.1",
    "Connection": "close"
    }
    request = urllib2.Request("https://whos.amung.us/pingjs/?k=26o15uz81d", headers=request_headers)
    response = urllib2.urlopen(request).read()
    #tempo_delay = 1
    #xbmc.sleep(tempo_delay*1000)
contador()
...
def vip():
    ...
    if tipo_servidor=='OnePlay':
       url = 'http://psrv.io:80/get.php?username=%s&password=%s&type=m3u_plus&output=%s'%(username_vip,password_vip,saida_transmissao)
...
def SKindex():
    addon_log("SKindex")
    getData('https://pastebin.com/','')
    vip()
    addDir(menu_configuracoes,'',41,icon_config,FANART,(desc_configuracoes),'','','')
                ...
                if len(url) > 1:
                    ...
                    if isXMLSource:
                            if not regexs == None: #<externallink> and <regex>
                                ...
                                #addLink(url[0],name.encode('utf-8', 'ignore')+  '[COLOR aquamarine]build XML[/COLOR]',thumbnail,fanArt,desc,genre,date,True,None,regexs,total)
                            else:
...
def urlsolver(url):
    ...
    ##genesis_url = 'https://raw.githubusercontent.com/lambda81/lambda-addons/master/plugin.video.genesis/commonresolvers.py'
        genesis_url = 'https://raw.githubusercontent.com/OnePlayHD/OneRepo/master/plugin.video.genesis/commonresolvers.py'
...
#            print 'parentalblockedpin',parentalblockedpin
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from OnePlay Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
            ...
            if not name in FAV:
                ...
                ##contextMenu.append(('Adicionar [COLOR aquamarine]OnePlay[/COLOR]','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
#                print 'setting as folder in link'
        ...
        if playlist:
            if addon.getSetting('add_playlist') == "false" and '$$LSPlayOnlyOne$$' not in playlist[0] :
                ...
                name = name + '[COLOR aquamarine] (' + str(len(playlist)) + ' itens)[/COLOR]'
                play_list = True
        else:
            u += "url="+urllib.quote_plus(url)+"&mode="+mode
        if regexs:
